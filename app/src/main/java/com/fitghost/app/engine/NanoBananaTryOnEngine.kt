...
class NanoBananaTryOnEngine(private val client: OkHttpClient = createHttpClient()) : TryOnEngine {
    ...
    override suspend fun renderPreview(
            ...
            withContext(Dispatchers.IO) {
                ...
                try {
                    ...
                    val requestBody =
                            buildGeminiVirtualTryOnRequest(modelPng, clothingPngs, systemPrompt)

                    Log.d(
                            TAG,
                            "Calling Google Gemini API for virtual try-on (model: gemini-2.5-flash-image-preview, modelImages=1, clothes=${clothingPngs.size})"
                    )
                    ...
                    client.newCall(request).execute().use { response ->
                        if (!response.isSuccessful) {
                            ...
                            val errorMsg =
                                    "Google Gemini API HTTP ${response.code}: ${response.message} - $errorBody"
                            Log.e(TAG, errorMsg)
    ...
    /** Google Gemini API용 가상 피팅 요청 본문 생성 공식 문서 예제 형식을 따름 */
    ...
    /** 가상 피팅을 위한 프롬프트 생성 전문적인 이커머스 패션 사진 스타일로 요청 */
    private fun buildVirtualTryOnPrompt(systemPrompt: String?, clothingCount: Int): String {
        ...
        return TryOnPromptBuilder.buildUserInstruction(
                hasModel = true,
                clothingCount = clothingCount
        )
    }

    /** Google Gemini API 응답에서 생성된 이미지 추출 표준 Gemini API 응답 구조를 파싱 */
    private fun parseGeminiResponse(responseJson: String): ByteArray {
        return try {
            ...
            throw IllegalStateException(
                    "Failed to extract image from Gemini response: ${e.message}",
                    e
            )
        }
    }
